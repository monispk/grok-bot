...
const stored = () =>
  page.evaluate(() =>
    JSON.parse(localStorage.getItem("grok-bot:history") || "[]"),
  );
...
await check(
  "the welcome arrives one message at a time, pinned to the bottom",
  async () => {
    await page.goto(APP);
    ...
    const done = await settle(async () => {
      const s = await page.evaluate(() => {
        ...
        return {
          ...
          total: JSON.parse(localStorage.getItem("grok-bot:history") || "[]")
            .length,
        };
      });
      if (!s) return false;
    ...
    assert.ok(
      counts.size > 3,
      `messages appeared in ${counts.size} steps, so they were not staged`,
    );
    assert.ok(
      elapsed < 8000,
      `the welcome took ${elapsed}ms, which is a wait rather than a flourish`,
    );
    assert.ok(
      worstGap < 24,
      `drifted ${Math.round(worstGap)}px from the bottom; the rider would have to scroll`,
    );

    const kinds = await page.evaluate(() =>
      [...document.querySelector(".scroll").children].map((c) =>
        c.querySelector("img.photo")
          ? "IMG"
          : c.querySelector(".voice")
            ? "AUD"
            : "TXT",
      ),
    );
...
await check(
  "a wrong document is refused, and its voice note stays put",
  async () => {
    await primeAt(3, [
      {
        role: "assistant",
    ...
    const stillThere = await page.evaluate(
      () => !!document.querySelector(".voice audio"),
    );
    assert.ok(stillThere, "the voice note vanished after appearing");

    const step = await page.evaluate(
      () => JSON.parse(localStorage.getItem("grok-bot:flow")).step,
    );
...
await check("a second wrong document is refused again", async () => {
  ...
  const twice = await settle(async () => {
    ...
    return (
      h.filter((m) => (m.content || "").includes("nahi lag rahi")).length >= 2
    );
  });
  assert.ok(twice, "the second attempt got no answer at all");
});

/** Holds the button down long enough for MediaRecorder to emit real bytes. */
...
await check("a spoken answer is transcribed and answered", async () => {
  await primeAt(3, [
    {
      role: "assistant",
      content:
  ...
  // The rider's own clip appears immediately, pending, with a player.
  ...
  assert.ok(shown, "the transcript was never shown under the clip");
  ...
  const answered = await settle(async () => {
    ...
    return i >= 0 && h.length > i + 1 && h[i + 1].role === "assistant";
  });
  assert.ok(answered, "the transcript was not sent on to the bot");
...
await check(
  "the transcript survives a reload, though the clip cannot",
  async () => {
    await page.reload();
...
await check("the name must be typed, not spoken", async () => {
  ...
  const told = await settle(async () =>
    (await stored()).some((m) =>
      (m.content || "").includes("likh kar bhejein"),
    ),
  );
  ...
  assert.ok(!named, `the transcript was taken as the rider's name (${named})`);
});
