...
const JC = {
  ...
  /**
   * Must be the URL registered on the merchant profile, exactly.
   *
   * It is never visited: MWALLET is a direct debit and the rider approves in
   * their own app. It only has to match — and when it did not, JazzCash
   * refused every payment as "insufficient merchant information" while
   * accepting the same credentials on Status Inquiry, which cost a day to
   * work out. Nothing needs to be served at this address.
   */
